"""
@author: James Thewlis - https://github.com/jamt9000/DVE
...
class HourglassNet(BaseModel):
    def __init__(self, block=ResidualBottleneckPreactivation, num_stacks=1, num_blocks=4, planes_conv1=64, planes_block=128, planes_hg=128, num_output_channels=64, use_group_norm=False, output_layer_weight_mul=1., scaled_norm_output=False):
    ...
    def forward(self, x, layer=-1, output_shape=(48,48)): # hack: layer is actually not used, just fro compatibility of my codes
        out = []
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
