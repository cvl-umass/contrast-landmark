...
from utils.visualization import keypoints_moco, plot_gallery
...
def parse_option():
    ...
    parser.add_argument('--random_network', action='store_true', help='initialize the network randomly')
    ...
    # model path and name  
    parser.add_argument('--ckpt_path', type=str) # path to store the models
...
def validate(val_loader, model, regressor, criterion, opt):
    ...
    with torch.no_grad():
        ...
        for idx, (input, _, target, index) in enumerate(val_loader):
            ...
            # visualization, opt.visualization, opt.vis_path
            if opt.vis_keypoints and idx <= 3:
                keypoints_moco(input, output, target, index, opt.vis_path) 
