...
def plot_gallery(images, input_image, outpath):
    ...
    ax.set_axis_off()
    fig.add_axes(ax)
    ax.imshow(norm_range(input_image).permute(1, 2, 0).cpu().numpy())
    plt.xticks(())
    plt.yticks(())
    plt.savefig(os.path.join(outpath, 'orig.png'), dpi=2*input_image.shape[1])
    plt.close()

    # todo save figures in the format: identity_dir/original_image, basis_0, basis_n ...
    for i in range(images.shape[0]):
        ...
        plt.xticks(())
        plt.yticks(())
        plt.savefig(os.path.join(outpath, '%d.png'%i), dpi=2*images[i].shape[0])
        plt.close()



def keypoints_moco(data, preds, targets, index, outpath):
    
    data = data.detach().cpu().clone()
    data_norm = norm_range(data)
    
    for ii, idx in enumerate(index):
        
        img = data_norm[ii].permute(1, 2, 0)
        imH, imW, imC = img.shape

        pred = preds[ii].detach().cpu().clone()
        target = targets[ii].detach().cpu().clone()

        pred[..., 0] = (pred[..., 0] + 1.) / 2. * (imW - 1)
        pred[..., 1] = (pred[..., 1] + 1.) / 2. * (imH - 1)

        target[..., 0] = (target[..., 0] + 1.) / 2. * (imW - 1)
        target[..., 1] = (target[..., 1] + 1.) / 2. * (imH - 1)

        fig, ax = plt.subplots()
        ax.imshow(img)
        ax.scatter(pred[:, 0], pred[:, 1], c='b')
        ax.scatter(target[:, 0], target[:, 1], c='g')
        plt.axis('off')
        plt.savefig(os.path.join(outpath, '%d.png'%idx.numpy()), bbox_inches='tight')
        plt.close()


def keypoints_animal(data, preds, targets, visible, index, outpath):
    
    data = data.detach().cpu().clone()
    data_norm = norm_range(data)
    
    # use different color for different parts
    np.random.seed(0)
    colormap = np.random.rand(15, 3)

    for ii, idx in enumerate(index):
        
        vis_array = visible[ii]
        ...
        # only visualize visible keypoints 
        vis = visible[ii]
        pred = pred[vis]
        target = target[vis]
        colors = colormap[vis]
        ...
        fig.add_axes(ax)
        ax.imshow(img)
        ax.scatter(pred[:, 0], pred[:, 1], c=colors, s=8, marker='.')
        plt.xticks(())
        plt.yticks(())
        plt.savefig(os.path.join(outpath, '%d_pred.png'%idx.numpy()), dpi=imH)
        plt.close()
        ...
        fig.add_axes(ax)
        ax.imshow(img)
        ax.scatter(target[:, 0], target[:, 1], c=colors, s=8, marker='.')
        plt.xticks(())
        plt.yticks(())
        plt.savefig(os.path.join(outpath, '%d_gt.png'%idx.numpy()), dpi=imH)
        plt.close()

    
def keypoints_animal_HR(data, preds, targets, visible, index, outpath, image_path):
    ...
    for ii, idx in enumerate(index):
        ...
        # print(image_path[ii])
        img = Image.open(image_path[ii]).convert('RGB')
        # use the same aspect ratio as the input to the network
        ...
        fig.add_axes(ax)
        ax.imshow(img)
        ax.scatter(pred[:, 0], pred[:, 1], c=colors, s=4, marker="+")
        plt.xticks(())
        plt.yticks(())
        plt.savefig(os.path.join(outpath, '%d_pred.png'%idx.numpy()), dpi=imH)
        plt.close()
        ...
        fig.add_axes(ax)
        ax.imshow(img)
        ax.scatter(target[:, 0], target[:, 1], c=colors, s=4, marker="+")
        plt.xticks(())
        plt.yticks(())
        plt.savefig(os.path.join(outpath, '%d_gt.png'%idx.numpy()), dpi=imH)
        plt.close()
